...
import { TCouponParams } from '@/components/types';
...
const UpdateCouponForm = ({ data }: { data: TCouponParams }) => {
  ...
  async function onSubmit(values: z.infer<typeof couponFormSchema>) {
    try {
      ...
      const updateCoupons = await updateCoupon({
        _id: data._id,
        updateData: {
          ...values,
          value: couponValue,
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Input
                    type="number"
                    placeholder="Nhập số lượng tối đa: 100"
                    {...field}
                    ...
                    {selectedCourses.length > 0 && (
                      <div className="flex flex-wrap gap-2 !mt-5">
                        {selectedCourses?.map((course) => (
                          <div
                            key={course.title}
                            className="inline-flex items-center gap-2 font-semibold text-sm cursor-pointer px-3 py-1 rounded-lg border borderDarkMode bgDarkMode"
                          >
                            <span>{course.title}</span>
                            <button
                              type="button"
                            ...
                            >
                              <IconClose className="size-5 text-gray-400 hover:text-gray-600"></IconClose>
                            </button>
                          </div>
                        ))}
                      </div>
